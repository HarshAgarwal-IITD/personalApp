...
export async function GET(req: NextRequest) {
  try {
    ...
    const todayTotal = todayExpenses.reduce((s: number, e: { amount: number }) => s + e.amount, 0);
    ...
    const monthTotal = monthExpenses.reduce((s: number, e: { amount: number }) => s + e.amount, 0);
